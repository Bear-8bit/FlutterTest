...
class APIRepository {
  ...
  Future<String> login(String accountID, String password) async {
    try {
      ...
      Response res = await api.sendRequest.post('/Auth/login', options: Options(headers: header('no token')), data: body);
      if (res.statusCode == 200) {
  ...
  Future<User> current(String token) async {
    try {
      Response res = await api.sendRequest.get('/Auth/current', options: Options(headers: header(token)));
  ...
  Future<List<CategoryModel>> getCategory(String accountID, String token) async {
    try {
      Response res = await api.sendRequest.get('/Category/getList?accountID=$accountID', options: Options(headers: header(token)));
      return res.data.map((e) => CategoryModel.fromJson(e)).cast<CategoryModel>().toList();
    } catch (ex) {
      rethrow;
    }
  }

  Future<bool> addCategory(CategoryModel data, String accountID, String token) async {
    try {
      final body = FormData.fromMap({
        'name': data.name,
      ...
      Response res = await api.sendRequest.post('/addCategory', options: Options(headers: header(token)), data: body);
      if (res.statusCode == 200) {
  ...
  Future<bool> updateCategory(int categoryID, CategoryModel data, String accountID, String token) async {
    try {
      final body = FormData.fromMap({
        'id': categoryID,
        'name': data.name,
  ...
  Future<bool> removeCategory(int categoryID, String accountID, String token) async {
    try {
      final body =
  ...
  Future<List<ProductModel>> getProductByCateID(String accountID, String token, int categoryID) async {
    try {
      Response res = await api.sendRequest.get('/Product/getListByCatId?categoryID=$categoryID&accountID=$accountID',
          options: Options(headers: header(token)),
      );
      return res.data.map((e) => ProductModel.fromJson(e)).cast<ProductModel>().toList();
    }
    catch (ex) {
      print(ex);
      rethrow;
  ...
  Future<List<ProductModel>> getProductAdmin(String accountID, String token) async {
    try {
      Response res = await api.sendRequest.get('/Product/getListAdmin?accountID=$accountID', options: Options(headers: header(token)));
      return res.data.map((e) => ProductModel.fromJson(e)).cast<ProductModel>().toList();
    } catch (ex) {
      print(ex);
      rethrow;
  ...
  Future<bool> addProduct(ProductModel data, String token) async {
    try {
      ...
      Response res = await api.sendRequest.post('/addProduct', options: Options(headers: header(token)), data: body);
      if (res.statusCode == 200) {
  ...
  Future<bool> removeBill(
      String billID, String token) async {
    try {
      final body =
          FormData.fromMap({'billID': billID, });
